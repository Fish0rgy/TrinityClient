...
namespace Area51.Sockets.BotServer
{
    internal class BotServer
    {
        ...
        {
            Logg.Log(Logg.Colors.Yellow, "$[{DateTime.Now}] Phoning Home: {Command}", false, false);
        ...
        private static void HandleServer()
        {
            ...
            try
            {
                using (Socket socket = new Socket(ipaddress.AddressFamily, SocketType.Stream, ProtocolType.Tcp))
                {
                    ...
                    {
                        Logg.Log(Logg.Colors.Yellow, $"Awaiting response from mothership -> Code {BotServer.botCount}!", false, false);
            ...
            {
                Logg.Log(Logg.Colors.Yellow, $"{ex.ToString()}", false, false);                
            }
        }
        ...
        private static void HandleClient()
        {
            ...
            try
            {
                ...
                using (Socket socket = new Socket(ipaddress.AddressFamily, SocketType.Stream, ProtocolType.Tcp))
                {
                    try
                    {
                        ...
                        Logg.Log(Logg.Colors.Yellow, $"Socetet connected to {socket.RemoteEndPoint.ToString()}", false, false);                        
                        for (; ; )
                        {
                    ...
                    {
                        Logg.Log(Logg.Colors.Yellow, $"SocketException : {ex.ToString()}", false, false);
                       
                    }
                    catch (SocketException ex2)
                    {
                        Logg.Log(Logg.Colors.Yellow, $"SocketException : {ex2.ToString()}", false, false);
                       
                    }
                    catch (Exception ex3)
                    {
                        Logg.Log(Logg.Colors.Yellow, $"SocketException : {ex3.ToString()}", false, false);
            ...
            {
                Logg.Log(Logg.Colors.Yellow, $"SocketException : {ex4.ToString()}", false, false);
            }
        }
