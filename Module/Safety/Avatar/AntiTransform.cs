...
{
    class AntiTransform : BaseModule, OnObjectInstantiatedEvent
    {
        public AntiTransform() : base("Anti Transformers", "Limit Audio Recources", Main.Instance.Avatarbutton, null, true, false)
        {
        }
        public override void OnEnable()
        {
            MenuUI.Log("SAFETY: <color=green>Anti Transformers Enabled</color>");
            Main.Instance.OnObjectInstantiatedEvents.Add(this);
        }

        public override void OnDisable()
        {
            MenuUI.Log("SAFETY: <color=red>Anti Transformers Disabled</color>");
        ...
        {  
            ...
            List<Transform> transforms = MunchenAntiCrash.FindAllComponentsInGameObject<Transform>(instantiatedGameObject); 
            if (containsPrefabName == true)
            {
                ...
                for (int i = 0; i < transforms.Count; i++)
                {
                    ...
                    limitedTransforms = MunchenAntiCrash.ProcessTransform(transforms[i], limitedTransforms);

                }
                if (limitedTransforms > 0)
                { 
                    LogHandler.Log(LogHandler.Colors.Green, $"[AntiCrash] Removed {limitedTransforms} Bad Transforms");
                    LogHandler.LogDebug($"[Anti AviCrash] Removed {limitedTransforms} Bad Transforms");
                }
            }
            return true;
        }
